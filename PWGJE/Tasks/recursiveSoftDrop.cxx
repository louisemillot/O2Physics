...
struct MyCustomTask {
  ...
  void processMCTask(aod::McCollision const&, aod::McParticles const& mcParticles)
  {
  ...
  PROCESS_SWITCH(MyCustomTask, processMCTask, "process task for MC particles", false);

  void processDataTask(aod::Collision const& collision, aod::Tracks const& tracks)
{
    registry.fill(HIST("NEvents"), 0.5);
    
    // Convert tracks to FastJet format
    vector<PseudoJet> particles;
    ...
    for (const auto& jet : jets) {
        ...
        for (unsigned int i=0; i<ztg.size(); ++i) {
            double kT = ztg[i].first * ztg[i].second * rsd_jet.pt();
            LOGF(info, "%5d%14.4f%14.4f%14.4f%14.4f", i+1, ztg[i].first, ztg[i].second, rsd_jet.pt(), kT);
            registry.fill(HIST("zg"), ztg[i].first);
            registry.fill(HIST("thetag"), ztg[i].second);
            registry.fill(HIST("kT"), kT);

            if (ztg[i].second > 0 && kT > 0) {
                double ln_kt = TMath::Log(kT);
                double ln_inv_thetag = TMath::Log(1./ztg[i].second);
                registry.fill(HIST("h2_lnkt_vs_lnthetag"), ln_inv_thetag, ln_kt);
                double tf = 2.0 / (kT * ztg[i].second);
  ...
  PROCESS_SWITCH(MyCustomTask, processDataTask, "process task for data particles", true);
};
